...
class TaskItem {
  ...
  /// Рядок часу для відображення на картці (наприклад "20:21" або "14 травня")
...
class _HomeScreenState extends State<HomeScreen>
  ...
  /// Застосовує зміни після редагування завдання
  void _applyTaskEdit(TaskItem task, TaskData data) {
    setState(() {
      ...
      if (data.listId != null) task.listId = data.listId!;
    });
  }
  ...
  Future<void> _openAddTask() async {
    ...
    final localizedLists = _lists.map((l) {
      if (l.id == _inboxListId) return l.copyWith(name: context.t.home.inbox);
      return l;
    }).toList();
  ...
  /// Відкриває detail sheet при тапі на завдання
  Future<void> _openTaskDetail(TaskItem task) async {
    final detail = TaskDetailData(
      id: task.id,
      title: task.title,
    ...
    await showModalBottomSheet<void>(
      ...
      builder: (_) => TaskDetailBottomSheet(
        ...
        lists: localizedLists,
        listName: listName,
        onSaved: (data) => _applyTaskEdit(task, data),
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (ctx) {
        ...
        return AlertDialog(
          ...
          content: Text(t.home.deleteListMessage, textAlign: TextAlign.center),
          actions: [
            TextButton(
          ...
          if (_isDrawerOpen)
            AnimatedBuilder(
              ...
              builder: (_, _) => GestureDetector(
                ...
                child: Container(
                  color:
                  Colors.black.withValues(alpha: 0.35 * _drawerAnimation.value),
                ),
              ),
            ),
        ...
        child: Column(
          ...
          children: [
            Padding(
              ...
              child: Text(t.home.myLists, style: textTheme.titleLarge),
            ),
            ...
            Expanded(
              child: ListView.builder(
                ...
                itemCount:
                lists.where((l) => l.id != inboxListId).length,
                itemBuilder: (_, i) {
                  final userLists =
                  lists.where((l) => l.id != inboxListId).toList();
              ...
              children: [
                ...
                Text(
                  ...
                  style: textTheme.bodySmall?.copyWith(
                    color: task.dueTime != null ? AppColors.primary :
                                                  colors.textSecondary,
                    fontWeight: task.dueTime != null ? FontWeight.w600 :
                                                      FontWeight.w400,
                  ),
                ),
              ],
...
class _SwipableCompletedTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Dismissible(
      key: ValueKey('dismissible-completed-${task.id}'),
      direction: DismissDirection.endToStart,
      background: Container(
        alignment: Alignment.centerRight,
        padding: const EdgeInsets.only(right: 20),
...
class _EmptyStateView extends StatelessWidget {
  const _EmptyStateView({required this.title, super.key});
                          ...
                          return GestureDetector(
                            ...
                            child: AnimatedContainer(
                              ...
                              decoration: BoxDecoration(
                                ...
                                border: isSelected
                                    ? Border.all(
                                    color: _selectedColor, width: 1.5)
                                    : null,
                              ),
